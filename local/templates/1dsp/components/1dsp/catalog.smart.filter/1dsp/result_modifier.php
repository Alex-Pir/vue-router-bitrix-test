...
$arFilterUrl = explode('/', $arResult["FORM_ACTION"]);

//обрезаем action для того, чтобы ajax выполнялся нормально для случая, когда фильтр вне категории
if ($arFilterUrl[2] === 'filter')
{
    array_splice($arFilterUrl, 2, count($arFilterUrl) - 3);
    $arResult["FORM_ACTION"] = implode('/', $arFilterUrl);
}
