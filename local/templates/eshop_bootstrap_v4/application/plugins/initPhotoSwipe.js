    ...
    $.fn.initPhotoSwipe = function(options) {
        ...
        if (!$('.pswp').length) {;
            (function() {
                var pswp = '<div class="pswp" tabindex="-1" role="dialog" aria-hidden="true">' +
                    ...
                    '</div>' +
                    '<button class="pswp__button pswp__button--arrow--left" title="Предыдущая (Левая клавиша стрелка)"></button>' +
                    '<button class="pswp__button pswp__button--arrow--right" title="Следующая (Правая клавиша стрелка)"></button>' +
        ...
        if (!settings.items.length) {
            $elements.each(function(index, el) {
                ...
                if (!dataSizes) {
                    console.error("initPhotoSwipe: Нет атрибута 'data-size'='WIDTHxHEIGHT' у ссылки");
                    return;
                }
                if (!$link.attr("href").length) {
                    console.error("initPhotoSwipe: Нет атрибута 'href' у ссылки:");
                    return;
                }
